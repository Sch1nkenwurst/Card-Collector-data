...
async function search(){
  const name=$('#name').value.trim(), number=$('#number').value.trim().split('/')[0], lang=$('#language').value;
...
Object.assign(state,{autoScanning:false,recognizing:false,lastSample:null,stableFrames:0,autoCandidates:[],autoChosen:null,scanTimer:null});
const sampleCanvas=document.createElement('canvas'); sampleCanvas.width=32; sampleCanvas.height=44;
const sampleCtx=sampleCanvas.getContext('2d',{willReadFrequently:true});
const ocrLanguages={de:'deu',en:'eng',fr:'fra',it:'ita',es:'spa',pt:'por',ja:'jpn'};
...
function stopAutoScan(){state.autoScanning=false;clearInterval(state.scanTimer);state.scanTimer=null;$('#autoBtn').textContent='Automatisches Scannen starten';setLive('','Scanner aus');progress(0)}
...
function scanFrame(){
  ...
  state.lastSample=gray; state.stableFrames=(diff<5&&variance>350)?state.stableFrames+1:0;progress(Math.min(35,state.stableFrames*12));
  if(state.stableFrames>=3)recognizeCurrentCard();
}
...
async function findOcrCandidates(parsed,lang){
  ...
  if(!params.toString())return[];const res=await fetch(`https://api.tcgdex.net/v2/${lang}/cards?${params}`);if(!res.ok)throw new Error('Kartendatenbank nicht erreichbar');let list=await res.json();
...
function showAutoCandidate(card,confidence){state.autoChosen=card;$('#autoCandidate').innerHTML=`<div class="auto-card"><img src="${imageUrl(card)}" alt=""><span class="eyebrow">ERKANNTER TREFFER</span><h2>${esc(card.name)}</h2><p>${esc(setName(card))} · ${esc(card.localId||'–')} · ${langNames[$('#language').value]}</p></div>`;$('#confidence').textContent=confidence+' %';$('#confirmDialog').showModal();setLive('busy','Bitte bestätigen')}
$('#acceptBtn').onclick=()=>{const card=state.autoChosen,lang=$('#language').value;if(!card)return;const existing=state.cards.find(c=>c.tcgdexId===card.id&&c.language===lang&&c.condition==='NM'&&c.variant==='Normal');if(existing)existing.quantity+=1;else state.cards.unshift({uid:crypto.randomUUID(),tcgdexId:card.id,name:card.name,set:setName(card),setId:card.set?.id||'',number:card.localId||'',language:lang,condition:'NM',variant:'Normal',quantity:1,price:marketPrice(card),cardmarketId:card.variants?.find?.(v=>v.thirdParty?.cardmarket)?.thirdParty?.cardmarket||card.thirdParty?.cardmarket||'',image:imageUrl(card),photo:state.photo,addedAt:new Date().toISOString()});persist();resumeAfterDecision(`${card.name} übernommen. Karte entfernen und nächste einlegen.`)};
...
function resumeAfterDecision(message){setStatus(message);setLive('on','Nächste Karte');progress(0);state.lastSample=null;state.stableFrames=0;state.recognizing=true;setTimeout(()=>{state.recognizing=false},2200)}
